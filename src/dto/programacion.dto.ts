...
export const ProgramacionItemSchema = z.object({
  ...
  programacion: z
    .string()
    .min(1, 'Programación es requerida')
    .max(255, 'Programación no puede exceder 255 caracteres'),
  hora_partida: z
    .string()
    .regex(
  ...
  estado_programacion: z
    .string()
    .max(255, 'Estado de programación no puede exceder 255 caracteres')
    .optional()
    .transform((val) => (val === '' ? undefined : val)),
  comentarios: z
    .string()
    .max(500, 'Comentarios no pueden exceder 500 caracteres')
    .optional()
    .transform((val) => (val === '' ? undefined : val)),
  punto_partida_ubigeo: z
    .string()
    .min(1, 'Ubigeo de partida es requerido')
    .max(255),
  punto_partida_direccion: z
    .string()
    .min(1, 'Dirección de partida es requerida')
    .max(255),
  punto_llegada_ubigeo: z
    .string()
    .min(1, 'Ubigeo de llegada es requerido')
    .max(255),
  punto_llegada_direccion: z
    .string()
    .min(1, 'Dirección de llegada es requerida')
    .max(255),
  peso: z
    .string()
    .max(255, 'Peso no puede exceder 255 caracteres')
    .optional()
    .transform((val) => (val === '' ? undefined : val)),
  id_proyecto: z
    .number()
    .int()
    .positive('ID de proyecto debe ser un número positivo')
    .optional(),
  id_subproyecto: z
    .number()
    .int()
    .positive('ID de subproyecto debe ser un número positivo')
    .optional(),
  numero_orden: z
    .string()
    .max(100, 'Número de orden no puede exceder 100 caracteres')
    .optional()
    .transform((val) => (val === '' ? undefined : val)),
});
...
export const CreateProgramacionSchema = z.object({
  data: z
    .array(ProgramacionItemSchema)
    .min(1, 'Debe proporcionar al menos un registro')
    .max(1000, 'No se pueden procesar más de 1000 registros a la vez'),
});
...
export type ProgramacionResponseDto = z.infer<
  typeof ProgramacionResponseSchema
>;
