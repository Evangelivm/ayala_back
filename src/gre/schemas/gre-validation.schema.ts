...
const RucSchema = z
  .string()
  .regex(/^(10|20)\d{9}$/, 'RUC debe tener 11 dígitos y empezar con 10 o 20');
...
const UbigeoSchema = z
  .string()
  .regex(/^\d{6}$/, 'UBIGEO debe tener exactamente 6 dígitos');

// Validación de placa de vehículo (formato ABC-123 o ABC123)
const PlacaSchema = z
  .string()
  .regex(/^[A-Z]{3}-?\d{3}$/, 'Placa debe tener formato ABC-123 o ABC123')
  .max(8);

// Validación de licencia (formato Q + 8 dígitos)
const LicenciaSchema = z
  .string()
  .regex(/^Q\d{8}$/, 'Licencia debe tener formato Q seguido de 8 dígitos');
...
const ItemGreSchema = z.object({
  ...
  codigo: z
    .string()
    .max(30, 'Código máximo 30 caracteres')
    .min(1, 'Código es obligatorio'),
  descripcion: z
    .string()
    .max(250, 'Descripción máximo 250 caracteres')
  ...
  tipo_de_igv: z
    ...
    .int()
    .min(1)
    .max(8, 'Tipo de IGV debe estar entre 1 y 8'),
...
export const GreValidationSchema = z
  .object({
    operacion: z.literal('generar_guia', {
      errorMap: () => ({
        message: 'Operación debe ser exactamente "generar_guia"',
      }),
    }),
    ...
    serie: z
      ...
      .regex(/^T\d{3}$/, 'Serie debe tener formato T001-T999'),
    ...
    cliente_denominacion: z
      .string()
      .max(100, 'Denominación máximo 100 caracteres')
      .min(1, 'Denominación es obligatoria'),
    cliente_direccion: z
      .string()
      .max(100, 'Dirección máximo 100 caracteres')
      .min(1, 'Dirección es obligatoria'),
    ...
    porcentaje_de_igv: z.number().default(18.0),
    ...
    parametros_adicionales: z.object({
      ...
      DIRECCION_PARTIDA: z
        .string()
        .max(100, 'Dirección de partida máximo 100 caracteres')
        .min(1, 'Dirección de partida es obligatoria'),
      UBIGEO_LLEGADA: UbigeoSchema,
      DIRECCION_LLEGADA: z
        .string()
        .max(100, 'Dirección de llegada máximo 100 caracteres')
        .min(1, 'Dirección de llegada es obligatoria'),
      NUMERO_DOCUMENTO_CONDUCTOR: DniSchema,
      TIPO_DOCUMENTO_CONDUCTOR: z.literal('1', {
        errorMap: () => ({
          message: 'Tipo documento conductor debe ser "1" (DNI)',
        }),
      }),
      NOMBRES_CONDUCTOR: z
        .string()
        .max(50, 'Nombres conductor máximo 50 caracteres')
        .min(1, 'Nombres conductor es obligatorio'),
      APELLIDOS_CONDUCTOR: z
        .string()
        .max(50, 'Apellidos conductor máximo 50 caracteres')
        .min(1, 'Apellidos conductor es obligatorio'),
      NUMERO_LICENCIA: LicenciaSchema,
      NUMERO_DOCUMENTO_TRANSPORTE: RucSchema,
      TIPO_DOCUMENTO_TRANSPORTE: z.literal('6', {
        errorMap: () => ({
          message: 'Tipo documento transporte debe ser "6" (RUC)',
        }),
      }),
      RAZON_SOCIAL_TRANSPORTE: z
        .string()
        .max(100, 'Razón social transporte máximo 100 caracteres')
        .min(1, 'Razón social transporte es obligatoria'),
      NUMERO_MTCVC: z
        .string()
        .max(9, 'Número MTCVC máximo 9 dígitos')
        .regex(/^\d{1,9}$/, 'NUMERO_MTCVC debe ser numérico'),
    }),
  })
  .refine(
    (data) => {
