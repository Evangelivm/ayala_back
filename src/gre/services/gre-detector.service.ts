...
export class GreDetectorService {
  ...
  async detectCompleteRecords() {
    try {
      ...
      if (completeRecords.length > 0) {
        this.logger.log(
          `Encontrados ${completeRecords.length} registros para validar`,
        );
        ...
        for (const record of completeRecords) {
          try {
            ...
            if (isValid) {
              ...
              this.logger.debug(
                `Registro ${record.id_guia} no cumple validaciones, omitiendo...`,
              );
            }
          } catch (error) {
            this.logger.error(
              `Error procesando registro ${record.id_guia}:`,
              error,
            );
          }
        }
  ...
  private async validateGreRecord(record: any): Promise<boolean> {
    try {
      ...
      if (![7, 8].includes(record.tipo_de_comprobante)) {
        this.logger.debug(
          `Registro ${record.id_guia}: tipo_de_comprobante debe ser 7 u 8`,
        );
        return false;
      }
      ...
      if (tipoRemitente && !record.serie.startsWith('T')) {
        this.logger.debug(
          `Registro ${record.id_guia}: GRE Remitente debe iniciar con T`,
        );
        return false;
      }
      ...
      if (tipoTransportista && !record.serie.startsWith('V')) {
        this.logger.debug(
          `Registro ${record.id_guia}: GRE Transportista debe iniciar con V`,
        );
        return false;
      }
      ...
      if (
        ...
        this.logger.debug(
          `Registro ${record.id_guia}: faltan datos de cliente`,
        );
        return false;
      }
      ...
      if (!record.peso_bruto_total || record.peso_bruto_total <= 0) {
        this.logger.debug(
          `Registro ${record.id_guia}: peso_bruto_total inválido`,
        );
        return false;
      }
      ...
      if (
        ...
        this.logger.debug(
          `Registro ${record.id_guia}: peso_bruto_unidad_de_medida inválida`,
        );
        return false;
      }
      ...
      if (
        ...
        this.logger.debug(
          `Registro ${record.id_guia}: datos de partida incompletos`,
        );
        return false;
      }
      ...
      if (
        ...
        this.logger.debug(
          `Registro ${record.id_guia}: datos de llegada incompletos`,
        );
        return false;
      }
  ...
  private validateGreRemitente(record: any): boolean {
    ...
    if (!record.motivo_de_traslado) {
      this.logger.warn(
        `❌ Registro ${record.id_guia}: falta motivo_de_traslado (obligatorio para GRE Remitente)`,
      );
      return false;
    }
    ...
    if (!motivosValidos.includes(record.motivo_de_traslado)) {
      this.logger.warn(
        `❌ Registro ${record.id_guia}: motivo_de_traslado inválido`,
      );
      return false;
    }
    ...
    if (!record.numero_de_bultos || record.numero_de_bultos < 1) {
      this.logger.warn(
        `❌ Registro ${record.id_guia}: falta numero_de_bultos (obligatorio para GRE Remitente)`,
      );
      return false;
    }
    ...
    if (
      ...
      this.logger.warn(
        `❌ Registro ${record.id_guia}: falta tipo_de_transporte (obligatorio para GRE Remitente)`,
      );
      return false;
    }
    ...
    if (record.tipo_de_transporte === '01') {
      ...
      if (
        ...
        this.logger.warn(
          `❌ Registro ${record.id_guia}: transportista_documento_tipo debe ser 6 (RUC)`,
        );
        return false;
      }
      ...
      if (
        ...
        this.logger.warn(
          `❌ Registro ${record.id_guia}: transportista_documento_numero inválido (debe ser RUC de 11 dígitos)`,
        );
        return false;
      }
      ...
      if (!record.transportista_denominacion) {
        this.logger.warn(
          `❌ Registro ${record.id_guia}: falta transportista_denominacion`,
        );
        return false;
      }
    ...
    if (record.tipo_de_transporte === '02') {
      ...
      if (
        ...
        this.logger.warn(
          `❌ Registro ${record.id_guia}: conductor_documento_tipo inválido (actual: ${record.conductor_documento_tipo})`,
        );
        return false;
      }
      ...
      if (!record.conductor_documento_numero) {
        this.logger.warn(
          `❌ Registro ${record.id_guia}: falta conductor_documento_numero`,
        );
        return false;
      }
      ...
      if (!record.conductor_nombre || !record.conductor_apellidos) {
        this.logger.warn(
          `❌ Registro ${record.id_guia}: faltan datos del conductor (nombre: ${record.conductor_nombre}, apellidos: ${record.conductor_apellidos})`,
        );
        return false;
      }
      ...
      if (
        ...
        this.logger.warn(
          `❌ Registro ${record.id_guia}: conductor_numero_licencia inválida (actual: ${record.conductor_numero_licencia})`,
        );
        return false;
      }
    ...
    this.logger.log(
      `✅ Registro ${record.id_guia}: validación GRE Remitente exitosa`,
    );
    return true;
  }
  ...
  private validateGreTransportista(record: any): boolean {
    ...
    if (
      ...
      this.logger.debug(
        `Registro ${record.id_guia}: conductor_documento_tipo inválido (obligatorio para GRE Transportista)`,
      );
      return false;
    }
    ...
    if (!record.conductor_documento_numero) {
      this.logger.debug(
        `Registro ${record.id_guia}: falta conductor_documento_numero (obligatorio para GRE Transportista)`,
      );
      return false;
    }
    ...
    if (
      ...
      this.logger.debug(
        `Registro ${record.id_guia}: faltan datos del conductor (obligatorios para GRE Transportista)`,
      );
      return false;
    }
    ...
    if (
      ...
      this.logger.debug(
        `Registro ${record.id_guia}: conductor_numero_licencia inválida (obligatoria para GRE Transportista)`,
      );
      return false;
    }
    ...
    if (
      ...
      this.logger.debug(
        `Registro ${record.id_guia}: destinatario_documento_tipo inválido (obligatorio para GRE Transportista)`,
      );
      return false;
    }
    ...
    if (!record.destinatario_documento_numero) {
      this.logger.debug(
        `Registro ${record.id_guia}: falta destinatario_documento_numero (obligatorio para GRE Transportista)`,
      );
      return false;
    }
    ...
    if (!record.destinatario_denominacion) {
      this.logger.debug(
        `Registro ${record.id_guia}: falta destinatario_denominacion (obligatorio para GRE Transportista)`,
      );
      return false;
    }
    ...
    if (
      ...
      this.logger.debug(
        `Registro ${record.id_guia}: tuc_vehiculo_principal con formato inválido`,
      );
      return false;
    }

    this.logger.debug(
      `Registro ${record.id_guia}: validación GRE Transportista exitosa`,
    );
    return true;
  }
  ...
  private async processCompleteRecord(record: any) {
    try {
      ...
      this.logger.log(
        `Registro ${record.id_guia} enviado a Kafka con estado PENDIENTE`,
      );
    } catch (error) {
  ...
  private transformRecordToNubefactApi(record: any) {
    ...
    console.log(
      '   - fecha_de_inicio_de_traslado (raw):',
      record.fecha_de_inicio_de_traslado,
    );
    console.log(
      '   - typeof fecha_de_emision:',
      typeof record.fecha_de_emision,
    );
    ...
    const formatDate = (date: Date | string) => {
      ...
      console.log(
        `📅 [DETECTOR] formatDate - Input: ${date} → UTC+5: ${dateUTC.format('YYYY-MM-DD')} → Formatted: ${formatted}`,
      );
    ...
    if (
      record.documento_relacionado &&
      record.documento_relacionado.length > 0
    ) {
      payload.documento_relacionado = record.documento_relacionado.map(
        (doc: any) => ({
          tipo: doc.tipo,
          serie: doc.serie,
    ...
    if (
      record.vehiculos_secundarios &&
      record.vehiculos_secundarios.length > 0
    ) {
      payload.vehiculos_secundarios = record.vehiculos_secundarios
        .slice(0, 2)
        .map((vehiculo: any) => ({
    ...
    if (
      record.conductores_secundarios &&
      record.conductores_secundarios.length > 0
    ) {
      payload.conductores_secundarios = record.conductores_secundarios
        .slice(0, 2)
        .map((conductor: any) => ({
