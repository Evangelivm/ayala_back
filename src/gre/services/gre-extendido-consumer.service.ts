...
export class GreExtendidoConsumerService {
  ...
  async handleGreRequest(
    ...
    try {
      this.logger.log(
        `📥 Mensaje recibido de gre-extendido-requests: ${JSON.stringify(message).substring(0, 100)}...`,
      );

      const { id, recordId, data } = message;

      this.logger.log(
        `Procesando GRE Extendido request: ${id} para registro ${recordId}`,
      );
      ...
      if (currentRecord.estado_gre !== 'PENDIENTE') {
        this.logger.warn(
          `⚠️ Registro extendido ${recordId} ya fue procesado (estado: ${currentRecord.estado_gre}), ignorando mensaje duplicado`,
        );
        return;
      }
      ...
      if (nubefactResponse.success) {
        this.logger.log(
          `API generar_guia exitosa para registro extendido ${recordId}`,
        );
        ...
        await this.greExtendidoPolling.startPolling(
          recordId,
          id,
        ...
        this.logger.error(
          `Error en API generar_guia para registro extendido ${recordId}:`,
        ...
        await this.greExtendidoProducer.sendToFailed(
          id,
          recordId,
          nubefactResponse.error,
          data,
      ...
      try {
        ...
        this.logger.error(
          'Error actualizando estado extendido a FALLADO:',
          updateError,
        );
      }
    }
  ...
  async handleGreProcessing(
    ...
    try {
      ...
      this.logger.debug(
        `Procesando mensaje extendido en processing: ${id} para registro ${recordId}`,
      );
      ...
      if (record?.estado_gre === 'COMPLETADO') {
        this.logger.log(
          `Registro extendido ${recordId} ya completado, deteniendo procesamiento`,
        );
        await this.greExtendidoPolling.stopPolling(recordId);
      }
    } catch (error) {
      this.logger.error(
        'Error procesando mensaje extendido de processing:',
        error,
      );
    }
  }
  ...
  async handleGreResponse(
    ...
    try {
      ...
      this.logger.log(
        `Procesando respuesta extendida: ${id} para registro ${recordId}, estado: ${status}`,
      );
      ...
      if (status === 'success' && nubefact_response) {
        ...
        if (pdf_url && xml_url && cdr_url) {
          ...
          if (currentRecord?.estado_gre === 'COMPLETADO') {
            this.logger.log(
              `✅ Registro extendido ${recordId} ya fue completado por polling, omitiendo actualización duplicada`,
            );
            await this.greExtendidoPolling.stopPolling(recordId);
            return;
          }
          ...
          this.logger.log(
            `Registro extendido ${recordId} completado con todos los enlaces (vía consumer)`,
          );
          ...
          if (guiaCompletada.identificador_unico) {
            ...
            if (programacionTecnica) {
              ...
              this.logger.log(
                `📡 WebSocket emitido para prog-tecnica extendida desde consumer: ${guiaCompletada.identificador_unico}`,
              );
            } else {
              this.logger.warn(
          ...
          this.logger.debug(
            `Registro extendido ${recordId} aún sin todos los enlaces, continuando polling`,
          );
        }
      } else if (status === 'error') {
        this.logger.error(
          `Error en respuesta extendida para registro ${recordId}:`,
          error,
        );
  ...
  private async callNubefactGenerarGuia(greData: any) {
    try {
      ...
      console.log(
        `\n🔍 [NUBEFACT-PAYLOAD] greData.items antes de enviar:`,
        JSON.stringify(greData.items, null, 2),
      );
  ...
  async getConsumerStats() {
    try {
      ...
      const completados =
        await this.prismaService.guia_remision_extendida.count({
      ...
      const pollingActivos =
        await this.greExtendidoPolling.getActivePollingCount();
      ...
      this.logger.error(
        'Error obteniendo estadísticas extendidas del consumer:',
        error,
      );
      throw error;
    }
  ...
  async retryFailedRecords(): Promise<void> {
    try {
      ...
      this.logger.log(
        `Reintentando ${failedRecords.length} registros extendidos fallidos`,
      );
      ...
      for (const record of failedRecords) {
        try {
          ...
          this.logger.log(
            `Registro extendido ${record.id_guia} reseteado para reintento`,
          );
        } catch (error) {
          this.logger.error(
            `Error reseteando registro extendido ${record.id_guia}:`,
            error,
          );
        }
      }
      ...
      this.logger.error(
        'Error en retry de registros extendidos fallidos:',
        error,
      );
      throw error;
    }
