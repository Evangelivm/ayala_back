...
export class OrdenServicioController {
  ...
  async obtenerTipoCambio() {
    try {
      const tipoCambio =
        await this.ordenServicioService.obtenerTipoCambioSunat();
      return {
        success: true,
  ...
  async create(
    @Body(new ZodValidationPipe(CreateOrdenServicioSchema))
    createOrdenServicioDto: CreateOrdenServicioDto,
    @Request() req: any,
  ) {
    try {
      const usuarioId =
        createOrdenServicioDto.registrado_por || req.user?.id || 1;
  ...
  async update(
    ...
    try {
      const usuarioId =
        updateOrdenServicioDto.registrado_por || req.user?.id || 1;
  ...
  async removeOperacion(@Param('id') id: string) {
    try {
      await this.ordenServicioService.removeDocumento(+id, 'operacion');
  ...
  async removeCotizacion(@Param('id') id: string) {
    try {
      await this.ordenServicioService.removeDocumento(+id, 'cotizacion');
  ...
  async removeFactura(@Param('id') id: string) {
    try {
      await this.ordenServicioService.removeDocumento(+id, 'factura');
  ...
  async removeRetencion(@Param('id') id: string) {
    try {
      await this.ordenServicioService.removeDocumento(+id, 'retencion');
  ...
  async aprobarContabilidad(@Param('id') id: string) {
    try {
      ...
      console.error(
        'Error aprobando orden de servicio para contabilidad:',
        error,
      );
      throw error;
    }
  ...
  async aprobarAdministrador(@Param('id') id: string) {
    try {
      ...
      console.error(
        'Error aprobando orden de servicio para administración:',
        error,
      );
      throw error;
    }
  ...
  async aprobarJefeProyecto(@Param('id') id: string) {
    try {
      ...
      console.error(
        'Error aprobando orden de servicio para jefe de proyecto:',
        error,
      );
      throw error;
    }
  ...
  async actualizarNumeroFactura(
    ...
    try {
      await this.ordenServicioService.actualizarNumeroFactura(
        +id,
        body.nro_factura,
      );
      return {
  ...
  async migrarEstados() {
    try {
      const resultado =
        await this.ordenServicioService.migrarOrdenesACompletada();
      return {
        success: true,
  ...
  async uploadFile(
    ...
    try {
      console.log(
        `📤 Iniciando subida de archivo para orden de servicio ID: ${id}`,
      );
  ...
  async uploadCotizacion(
    ...
    try {
      console.log(
        `📤 Iniciando subida de cotización para orden de servicio ID: ${id}`,
      );
      ...
      try {
        await this.ordenServicioService.updateCotizacionUrl(
          +id,
          result.fileUrl,
        );
        console.log(
      ...
      console.error(
        '❌ Error al subir cotización para orden de servicio:',
        error,
      );
      throw error;
    }
  ...
  async uploadFactura(
    ...
    try {
      console.log(
        `📤 Iniciando subida de factura para orden de servicio ID: ${id}`,
      );
  ...
  async uploadComprobanteRetencion(
    ...
    try {
      console.log(
        `📤 Iniciando subida de comprobante de retención para orden de servicio ID: ${id}`,
      );
      ...
      try {
        await this.ordenServicioService.updateComprobanteRetencionUrl(
          +id,
          result.fileUrl,
          nroSerie.trim(),
        );
      ...
      console.error(
        '❌ Error al subir comprobante de retención para orden de servicio:',
        error,
      );
      throw error;
    }
  ...
  async uploadMultifacturaFactura(
    ...
    const detalle =
      await this.ordenServicioService.getMultifacturaDetalle(+detalleId);
    ...
    await this.ordenServicioService.updateMultifacturaFileUrl(
      +detalleId,
      'factura',
      result.fileUrl,
    );
