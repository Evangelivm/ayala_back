...
export class CamionesService {
  ...
  async findAll(filters?: CamionFilterDto) {
    ...
    const empresaCodigos = [
      ...new Set(camiones.map((c) => c.empresa).filter(Boolean)),
    ] as string[];
    ...
    const data = camiones.map((c) => ({
      ...
      razon_social_empresa: c.empresa
        ? empresasMap.get(c.empresa) || null
        : null,
    }));
  ...
  async update(id: number, data: UpdateCamionDto) {
    ...
    if (data.capacidad_tanque !== undefined)
      updateData.capacidad_tanque = data.capacidad_tanque;
    if (data.id_tipo_combustible_preferido !== undefined)
      updateData.id_tipo_combustible_preferido =
        data.id_tipo_combustible_preferido;
    if (data.activo !== undefined) updateData.activo = data.activo;
    if (data.dni !== undefined) updateData.dni = data.dni;
    if (data.nombre_chofer !== undefined)
      updateData.nombre_chofer = data.nombre_chofer;
    if (data.apellido_chofer !== undefined)
      updateData.apellido_chofer = data.apellido_chofer;
    if (data.numero_licencia !== undefined)
